...
function displayBaseResults(data) {
    ...
    <div class="results-info-modern">
        <div class="info-badge">
            ...
            <span>Se encontraron <strong>${data.length}</strong> folios</span>
        </div>
    </div>
        ...
        <table id="base-table">
            <thead>
                <tr>
                    ...
                    <th><i class="fas fa-tools"></i> Instalación</th>
                    <th><i class="fas fa-ruler-combined"></i> Metraje</th>
                    <th><i class="fas fa-file-invoice"></i> Plantilla</th>
                    <th><i class="fas fa-poll"></i> Encuesta</th>
                </tr>
            </thead>
            <tbody>
`;
...
function selectFolioForEdit(folioId, index) {
    ...
    if (folioSeleccionado) {
        document.getElementById('edit-plantilla').value = folioSeleccionado.plantilla || '';
        document.getElementById('edit-encuesta').value = folioSeleccionado.encuesta || '';
...
function loadFoliosFromFirebase() {    
    // IMPORTANTE: Asegúrate de que diga "return new Promise"
    return new Promise((resolve, reject) => {
        let primeraCarga = true;

        db.collection('folios').orderBy('creado_en', 'desc').onSnapshot((snapshot) => {
            folios = []; // Limpiamos el array local
            ...
            snapshot.forEach(doc => {
                ...
                const folio = {
                    ...
                    plantilla: folioData.plantilla || '',
                    encuesta: folioData.encuesta || ''
                };
            ...
            // ESTO ES LO QUE ELIMINA EL ERROR DEL .THEN()
            if (primeraCarga) {
                primeraCarga = false;
                resolve(folios); 
            }
...
async function saveFolioToFirebase(folioData) {
    try {
        // 1. OBTENER EL ID DEL PENDIENTE (si existe)
        // Buscamos el ID que guardamos en el dataset del formulario de agregar
        const formAgregar = document.getElementById('folio-form');
        const editingId = formAgregar ? formAgregar.dataset.editingId : null;

        // 2. SI EXISTE UN ID, ELIMINAMOS EL DOCUMENTO VIEJO PRIMERO
        if (editingId) {
            console.log("Liquidando folio pendiente. Eliminando registro temporal:", editingId);
            await db.collection('folios').doc(editingId).delete();
            
            // Limpiamos el dataset para que no intente borrarlo de nuevo en el próximo guardado
            delete formAgregar.dataset.editingId; // Limpieza
        }

        // 3. PREPARAR EL NUEVO OBJETO (Folio Liquidado)
        const firebaseFolio = {
            tecnico_id: 1,
            ...
            // Forzamos 'no' porque esta función es para liquidar folios finales
            pendiente: 'no', 
...
document.getElementById('folio-form').addEventListener('submit', async function(e) {
    ...
    folio.plantilla = folio.plantilla || '';
    folio.encuesta = folio.encuesta || '';
    ...
    try {
        ...
        if (telefono_valor !== '' && telefono_valor !== '0' && telefono_valor !== '0000000000') {
            // 1. Obtenemos el ID que estamos editando del dataset del formulario
            ...
            if (!qTel.empty) {
            // 3. Verificamos si alguno de los documentos encontrados NO es el que estamos editando
                const esDuplicadoReal = qTel.docs.some(doc => doc.id !== editingId);

                if (esDuplicadoReal) {
                    throw new Error(`El teléfono "${telefono_valor}" ya está registrado por otro técnico o en otro folio.`);
                }
            }
        }
...
function validateFolioForm() {
    const form = document.getElementById('folio-form');
    // 1. Obtener el valor del selector de pendiente de forma segura
    const pendienteSelect = form.querySelector('select[name="pendiente"]');
    const esPendiente = pendienteSelect && pendienteSelect.value === 'si';
    ...
    if (esPendiente) {
        // Validación para folios que se están guardando como PENDIENTES
        const telefonoPendiente = form.querySelector('input[name="telefono_pendiente"]');
        if (telefonoPendiente) {
            const val = telefonoPendiente.value.trim();
            if (!val) {
        ...
        // Validación para folios NORMALES (Liquidados)
        ...
        if (folioInput) {
            const folioVal = folioInput.value.trim();
            if (!folioVal) {
                showFieldError(folioInput, 'El folio es requerido');
                isValid = false;
            } else if (!/^\d{9}$/.test(folioVal)) {
...
//<th>PENDIENTE</th>
    ...
    if (container) {
        ...
        const plantilla = f.plantilla || '';
        const encuesta = f.encuesta || '';
...
document.getElementById('edit-folio-form').addEventListener('submit', async function(e) {
    ...
    try {
        const updateObj = {
            ...
            pendiente: updatedData.pendiente || 'no',
        ...
        if (currentUser && currentUser.role !== 'tecnico') {
            updateObj.plantilla = updatedData.plantilla || '';
            updateObj.encuesta = updatedData.encuesta || '';
        }
...
async function buscarNominasFirebase(tipo, nombre, fechaDesde, fechaHasta) {
    try {        
        ...
        if (fechaDesde && fechaDesde.trim() !== '') {
            ...
            nominasFiltradas = nominasFiltradas.filter(n => {
                if (!n.fecha_desde) return false;
                
                // Comparar strings en formato YYYY-MM-DD
                return n.fecha_desde >= fechaDesdeISO;
            });
        }
        ...
        if (fechaHasta && fechaHasta.trim() !== '') {
            ...
            nominasFiltradas = nominasFiltradas.filter(n => {
                ...
                // Comparar strings en formato YYYY-MM-DD
                return n.fecha_hasta <= fechaHastaISO;
            });
        }
        ...
        if (nominasFiltradas.length > 0) return nominasFiltradas;
...
function displayFilteredFFTHTable(data) {
    ...
    data.forEach(f => {
        ...
        if (f.instalacion === 'AEREA') {
            ...
            else if (row.Metraje >= 201 && row.Metraje <= 350) row['Bajante aereo de 250m'] = 1;
        }
        if (f.instalacion === 'SUBTERRANEA') {
            ...
            else if (row.Metraje >= 201 && row.Metraje <= 350) row['Bajante subterraneo de 250m'] = 1;
        }
...
async function guardarFolioPendienteIndependiente() {
    // 1. Obtener valores
    ...
    // Ya no dependemos obligatoriamente del input de fecha si este falla
    const fechaInput = document.getElementById('fp-fecha').value;
    ...
    const btnGuardar = document.querySelector('[onclick="guardarFolioPendienteIndependiente()"]');
    if(btnGuardar) btnGuardar.disabled = true;
    ...
    try {
        ...
        if (fechaInput) {
            // Si hay fecha en el input, la procesamos con cuidado
            const partes = fechaInput.split('-');
            // Creamos la fecha usando año, mes (0-11) y día
            fechaFinal = new Date(partes[0], partes[1] - 1, partes[2], 12, 0, 0);
        } else {
            // Si el input falla o está vacío, usamos la fecha de HOY
            fechaFinal = new Date();
        }

        // Si por alguna razón la fecha sigue siendo inválida, usamos el tiempo actual del servidor
        const timestampFirebase = (isNaN(fechaFinal.getTime())) 
            ? firebase.firestore.FieldValue.serverTimestamp() 
            : firebase.firestore.Timestamp.fromDate(fechaFinal);

        const nuevoFolioPendiente = {
            tipo: tipo,
            cope: cope,
            fecha: timestampFirebase, // Formato nativo de Firebase
            telefono: telefono,
            comentarios_pendiente: comentarios,
            tecnico_nombre: currentUser.displayName || currentUser.name || 'Técnico',
            creado_en: firebase.firestore.FieldValue.serverTimestamp(),
            pendiente: 'si',
            folio_os: "PENDIENTE " + telefono,
            tarea: "PENDIENTE",
            distrito: "PENDIENTE",
            plantilla: "PENDIENTE",
            encuesta: "PENDIENTE",
            alfanumerico: "PENDIENTE"
        };

        // 4. GUARDAR EN FIREBASE
        await db.collection('folios').add(nuevoFolioPendiente);

        alert("✅ Folio pendiente guardado exitosamente.");
        cerrarModalFolioPendiente();
...
function verificarFolioPendienteAlLogin() {
    ...
    if (!currentUser || !folios || folios.length === 0) {
        console.log("No hay usuario o los folios aún no cargan.");
        return;
    }

    const nombreTecnicoActual = (currentUser.displayName || currentUser.name || "").trim().toLowerCase();
    console.log("Buscando pendientes para:", nombreTecnicoActual);

    // 2. Buscamos el folio pendiente
    const folioPendiente = folios.find(f => {
        const nombreEnFolio = (f.tecnico_nombre || f.tecnico || "").trim().toLowerCase();
    
        // Nueva lógica: Es pendiente si el campo folio_os empieza con "PENDIENTE"
        const esFormatoPendiente = f.folio && String(f.folio).startsWith('PENDIENTE');
        const valorPendiente = String(f.pendiente).toLowerCase();
        const esEstadoSi = (valorPendiente === 'si' || valorPendiente === 'true');

        return esEstadoSi && nombreEnFolio === nombreTecnicoActual && esFormatoPendiente;
    });

    // 3. Si se encuentra, ejecutar lógica de auto-relleno
    if (folioPendiente) {
        console.log("¡Folio pendiente encontrado!", folioPendiente);
        const idReferencia = folioPendiente.telefono || "Sin teléfono";
        
        // Pequeño delay para que no aparezca el alert justo al entrar
        setTimeout(() => {
            const respuesta = confirm(
                `HOLA ${currentUser.name.toUpperCase()}:\n\nTienes un folio registrado como PENDIENTE (Tel: ${idReferencia}).\n\n¿Deseas terminar de liquidarlo ahora mismo?`
            );

            if (respuesta) {
                if (typeof showSection === 'function') showSection('add');
                
                setTimeout(() => {
                    const form = document.getElementById('folio-form');
                    if (form) {
                        // Rellenamos los campos conocidos
                        if (form.querySelector('[name="tipo"]')) form.querySelector('[name="tipo"]').value = folioPendiente.tipo || 'FIBRA';
                        if (form.querySelector('[name="cope"]')) form.querySelector('[name="cope"]').value = folioPendiente.cope || '';
                        if (form.querySelector('[name="telefono"]')) form.querySelector('[name="telefono"]').value = folioPendiente.telefono || '';
                        
                        // Marcamos el selector de pendiente como 'no' porque lo vamos a liquidar
                        const pendSelect = form.querySelector('select[name="pendiente"]');
                        if (pendSelect) {
                            pendSelect.value = 'no';
                            if (typeof togglePendienteFields === 'function') togglePendienteFields(false);
                        }

                        // Guardamos el ID para borrarlo al finalizar
                        form.dataset.editingId = folioPendiente.id;
                        
                        // Comentarios informativos
                        if (form.querySelector('[name="comentarios"]')) {
                            form.querySelector('[name="comentarios"]').value = `Viene de pendiente: ${folioPendiente.comentarios_pendiente || ''}`;
                        }
                    }
                }, 800);
            }
        ...
        console.log("No se encontraron folios pendientes para este usuario.");
    }

    // Limpiamos rastro de versiones antiguas
...
async function entrarAlSistema(user) {    
    ...
    try {
        // LIMPIA EL CACHÉ LOCAL ANTES DE CARGAR PARA EVITAR DATOS VIEJOS
        localStorage.removeItem('folios_backup');
        await loadFoliosFromFirebase();
    } catch (error) {
        console.error("Error cargando folios:", error);
    }
    ...
    setTimeout(() => {
        ...
        // --- MEJORA AQUÍ ---
        // Si el array folios aún está vacío, esperamos un poco más antes de verificar
        if (folios.length === 0) {
            console.log("Esperando datos de Firebase para verificar pendientes...");
            setTimeout(() => {
                if (typeof verificarFolioPendienteAlLogin === 'function') {
                    verificarFolioPendienteAlLogin();
                }
            }, 2000); // Damos 2 segundos extra si la red es lenta
        } else {
            if (typeof verificarFolioPendienteAlLogin === 'function') {
                verificarFolioPendienteAlLogin();
            }
        }
    }, 1000); // Aumentamos a 1 segundo el primer delay para dar tiempo a la sincronización
} 
...
const TIEMPO_MAXIMO_MINUTOS = 20;
const TIEMPO_MAXIMO_MS = TIEMPO_MAXIMO_MINUTOS * 60 * 1000;

function resetIdleTimer() {
    // Solo actualizamos si hay un usuario logueado
    if (typeof currentUser !== 'undefined' && currentUser) {
        ultimaActividad = Date.now();
        // Guardamos en localStorage por si el usuario recarga la página
        localStorage.setItem('ultimaActividad', ultimaActividad);
    }
}

function verificarSesionInactiva() {
    if (typeof currentUser === 'undefined' || !currentUser) return;

    const ahora = Date.now();
    // Recuperamos la última actividad del almacenamiento o de la variable
    const guardada = localStorage.getItem('ultimaActividad') || ultimaActividad;
    const tiempoTranscurrido = ahora - parseInt(guardada);
...
function cerrarSesionInactiva() {    
    ...
    localStorage.removeItem('permisos_cache'); // Opcional: limpiar cache de permisos

    // 2. Avisar al usuario
    //alert("Tu sesión ha expirado por inactividad. Por favor, ingresa de nuevo.");

    // 3. Reset visual de la interfaz (Ajustado a tus IDs)
    const loginSection = document.getElementById('login-section');
    const appElement = document.getElementById('app');
    ...
    // 4. Forzar recarga para limpiar variables en memoria y estados de Firebase
    window.location.reload();
}

// --- ACTIVADORES (LISTENERS) ---

// 1. Monitorear clics, teclas y toques en pantalla
['mousedown', 'mousemove', 'keypress', 'touchstart', 'scroll'].forEach(evento => {
    window.addEventListener(evento, resetIdleTimer, false);
});

// 2. Revisar cada 10 segundos automáticamente
setInterval(verificarSesionInactiva, 10000);

// 3. Revisar INSTANTÁNEAMENTE cuando el usuario vuelve a la pestaña o desbloquea el cel
window.addEventListener('focus', verificarSesionInactiva);
...
// ======================
    ...
    // 3. Permite pegar del portapapeles
    // Lista de IDs que deben permitir pegar siempre
    const idsParaLiberar = ['username', 'password'];

    idsParaLiberar.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            // Aseguramos que el CSS permita la interacción
            el.style.userSelect = "text";
            el.style.webkitUserSelect = "text";

            // Permitir el evento de pegado explícitamente
            el.addEventListener('paste', (e) => {
                e.stopPropagation(); 
            // No usamos preventDefault para que el sistema siga su curso natural
                return true;
            });
        }
    });

    // 4. Configurar toggle de contraseña
